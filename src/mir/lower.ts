...
export function lowerToMIR(hir: HIRModule, sourceFile?: string): MIRModule {
  ...
  for (const f of hir.functions) {
    // Library functions with array parameters are templates: array values live in
    // NBT storage and need call-site bindings. Lower only the specialized copies
    // generated by the array-parameter monomorphizer below.
...
function lowerFunction(
  ...
  fn.params.forEach((p) => {
    ...
    if (p.type.kind === 'array' && arrayArgBindings?.has(p.name)) {
      ...
      // `return h` in array-returning library templates resolve instead of
      // falling through to the unresolved-ident guard.
...
function lowerExpr(
  ...
  switch (expr.kind) {
    ...
    case 'is_check': {
      const t = ctx.freshTemp()
      const ns = ctx.getNamespace()
      const mcType = entityTypeToMinecraftId(expr.entityType)
      const selector = expr.expr.kind === 'selector'
        ? withSelectorType(selectorToString((expr.expr as any).sel ?? expr.expr), mcType)
        : `@s[type=${mcType}]`
      ctx.emit({ kind: 'call', dst: t, fn: `__raw:execute store success score $ret __${ns} if entity ${selector}`, args: [] })
      return { kind: 'temp', name: t }
    }
...
function entityTypeToMinecraftId(entityType: EntityTypeName): string {
  const explicit: Partial<Record<EntityTypeName, string>> = {
    entity: 'minecraft:entity',
    Player: 'minecraft:player',
    Mob: 'minecraft:mob',
    HostileMob: 'minecraft:monster',
    PassiveMob: 'minecraft:animal',
    ArmorStand: 'minecraft:armor_stand',
    Item: 'minecraft:item',
    Arrow: 'minecraft:arrow',
  }
  const mapped = explicit[entityType]
  if (mapped) return mapped

  return `minecraft:${entityType.replace(/([a-z0-9])([A-Z])/g, '$1_$2').toLowerCase()}`
}

function withSelectorType(selector: string, mcType: string): string {
  if (selector.includes('type=')) return selector
  const bracket = selector.indexOf('[')
  if (bracket === -1) return `${selector}[type=${mcType}]`
  return `${selector.slice(0, bracket + 1)}type=${mcType},${selector.slice(bracket + 1)}`
}
