...
function parseWatchObjective(source: string): string | undefined {
  const tokens = new Lexer(source).tokenize()
  const ast = new Parser(tokens).parse('test')
  return ast.declarations[0]?.watchObjective
}
...
describe('@watch decorator', () => {
  test('parser records the watched objective on the function', () => {
    expect(parseWatchObjective(`
      @watch("rs.kills")
      fn on_kill() {}
    `)).toBe('rs.kills')
  })
