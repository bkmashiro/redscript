...
describe('Lexer', () => {
  describe('keywords', () => {
    it('recognizes all keywords', () => {
      const tokens = tokenize('fn let const if else while for foreach match return as at in is struct extends impl enum trigger namespace')
      expect(kinds(tokens)).toEqual([
        'fn', 'let', 'const', 'if', 'else', 'while', 'for', 'foreach', 'match',
        'return', 'as', 'at', 'in', 'is', 'struct', 'extends', 'impl', 'enum', 'trigger', 'namespace', 'eof'
      ])
    })
