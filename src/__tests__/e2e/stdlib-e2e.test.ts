...
const _EASING_SRC = fs.readFileSync(
...
const _VEC_SRC_FULL = fs.readFileSync(
  path.join(__dirname, '../../stdlib/vec.mcrs'),
  'utf-8',
)
...
// ---------------------------------------------------------------------------
// graph.mcrs — runtime  (DISABLED)
// NOTE: All graph runtime tests use `g = graph_add_edge(g, ...)` which
// triggers the MIR "Unresolved identifier" bug for array-returning functions.
// ---------------------------------------------------------------------------

// graph.mcrs runtime tests removed: all use `g = graph_add_edge(g, ...)`
// which triggers the MIR "Unresolved identifier" bug for array-returning
// functions.  Since makeRuntime is called at describe scope, the crash
// prevents the entire test suite from loading.
