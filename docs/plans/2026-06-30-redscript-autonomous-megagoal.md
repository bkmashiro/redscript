...
- 2026-06-30: Completed second parallel Spark batch for release-gate hardening. Expanded external-objective verifier contract tests in `src/__tests__/lir/verify.test.ts`, added static validator boundary tests for `function ... with storage` and `execute if score ... return run function ... with storage` in `src/__tests__/mc-validator-extra.test.ts`, and documented evidence labels (`compile-only`, `static-mc-validation`, `golden-artifact-shape`, `live-paper-oracle`) across the coverage matrix, live-oracle candidate map, and follow-up audit. Controller fixed one expected diagnostic string in the static-validator worker diff. Gates: `npm run build` passed; `npm test -- --selectProjects unit --runTestsByPath src/__tests__/golden-examples.test.ts src/__tests__/lir/verify.test.ts src/__tests__/mc-syntax.test.ts src/__tests__/mc-validator-coverage.test.ts src/__tests__/mc-validator-extra.test.ts src/__tests__/compile-all-skip-manifest.test.ts src/__tests__/coverage-matrix.test.ts src/__tests__/compile-all.test.ts --runInBand` passed 8 suites / 281 tests; `npm run validate-mc` passed 15 tests; `git diff --check` passed.
- 2026-06-30: Completed third parallel Spark batch for release evidence tightening. Upgraded `src/__tests__/golden-examples.test.ts` so restored public examples/templates now compile, emit artifact-shape checks, and statically validate every emitted non-comment `.mcfunction` command with `MCCommandValidator` (not live Paper proof). Refreshed `docs/plans/redscript-coverage-matrix.md` and `.json` to remove stale `known-blocker/compile-all-skip` feature rows after known-language-gap reached zero, and added a `coverage-matrix.test.ts` drift guard tying that claim to the skip manifest. Controller fixed Spark's nested `test.each` callback syntax. Gates: `npm run build` passed; `npm test -- --selectProjects unit --runTestsByPath src/__tests__/golden-examples.test.ts src/__tests__/coverage-matrix.test.ts src/__tests__/compile-all-skip-manifest.test.ts src/__tests__/compile-all.test.ts --runInBand` passed 4 suites / 131 tests; `npm run validate-mc` passed 15 tests; `python3 -m json.tool docs/plans/redscript-coverage-matrix.json >/tmp/redscript-coverage-matrix.json.check` passed; `git diff --check` passed.
- 2026-06-30: Completed compile-all static-MC correctness gate after user requested continued work with stronger correctness verification. RED: new `src/__tests__/compile-all-static-mc-validation.test.ts` failed on invalid emitted commands across the full non-skipped `.mcrs` corpus: stale raw examples (`tp @s @p[limit=1] 0 0 0`, particle position using `@s`) plus static-validator false negatives for function tags and macro `function ... with storage` forms. GREEN: fixed the two public example commands, taught `MCCommandValidator` to accept function tag IDs and both root/path macro storage forms, and added focused validator tests. Gate: `npm test -- --selectProjects unit --runTestsByPath src/__tests__/mc-validator-extra.test.ts src/__tests__/compile-all-static-mc-validation.test.ts --runInBand` passed 2 suites / 51 tests. This is `static-mc-validation` evidence, not live Paper proof. Live sanity check also passed against local Paper 1.21.4: `curl -fsS --max-time 2 http://localhost:25561/status` online, then `MC_CORE_REQUIRE_ONLINE=true npm run test:mc-core:live` passed 19/19 cases. Full local unit gate after the slice passed: `MC_OFFLINE=true npx jest --selectProjects unit --runInBand` = 287 suites / 5318 tests. Updated coverage matrix/guard so this static validation gate is recorded as product-readiness evidence.
