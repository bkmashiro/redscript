...
**Status:** tranche R2 currently includes conservative `score_min`/`score_max` self-no-op folding in `src/optimizer/lir/const_imm.ts`, and now also adjacent const-driven `score_set` + const-materialization + `score_min`/`score_max` folding when the const slot is single-use and window is deterministic.
